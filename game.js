...
function loop(t){if(state!=='playing')return;frameHeartbeat=performance.now();const dt=Math.min(.033,(t-last)/1000)*gameSpeed;last=t;try{loopStage='update';update(dt);loopStage='draw';draw();loopStage='idle'}catch(e){reportRuntimeError(e,loopStage);return}requestAnimationFrame(loop)}
...
function update(dt){elapsed+=dt;spawn-=dt;
 ...
 if(elapsed>=300&&!miniBoss&&!boss){spawnMiniBoss();}
...
if(player.hp<=0){if(player.shield){player.shield=false;player.hp=1;toast('护盾抵挡了致命伤害')}else{end();return}}}}
 ...
 if(miniBoss&&!miniBoss.dead)updateMiniBoss(dt);if(boss&&!boss.dead)updateBoss(dt);if(boss&&!boss.defeated&&boss.hp<=0){finishBoss();return;}
 ...
 for(const d of drops){let dd=dist(d,player);if(dd<player.magnet){let a=Math.atan2(player.y-d.y,player.x-d.x);let pullSpeed=480+Math.max(0,player.magnet-dd)*4;d.x+=Math.cos(a)*pullSpeed*dt;d.y+=Math.sin(a)*pullSpeed*dt}if(dist(d,player)<player.r+d.r){if(d.type==='xp')gainXp(d.v);else if(d.type==='power'){const choices=['damage','rate','heal'];const type=choices[Math.floor(Math.random()*choices.length)];if(type==='damage')player.damageMul*=1.12;else if(type==='rate')player.rateMul*=.9;else player.hp=Math.min(player.maxHp,player.hp+player.maxHp*.25);toast(type==='damage'?'强化核心：伤害 +12%':type==='rate'?'强化核心：攻击速度 +10%':'强化核心：恢复 25% 生命')}else if(d.type==='bomb'){for(const e of enemies){if(!e.dead&&dist(d,e)<110){e.hp-=d.v;if(e.hp<=0)kill(e)}}for(let i=0;i<24;i++)particles.push({x:d.x,y:d.y,vx:(Math.random()-.5)*300,vy:(Math.random()-.5)*300,life:.5});toast('爆裂核心：范围伤害')}else player.gold+=Math.ceil(d.v*(player.relics?.includes('greed')?1.25:1));d.dead=true}}drops=drops.filter(d=>!d.dead);
...
function shootWeapon(id,w){
  ...
  if(id==='lightning'){
    ...
    if(miniBoss&&!miniBoss.dead&&miniBoss.spawned&&targets.length<(w.count||3))targets.push(miniBoss);if(boss&&!boss.dead&&boss.spawned&&targets.length<(w.count||3))targets.push(boss);
    for(const t of targets){t.hp-=damage;particles.push({x:t.x,y:t.y,vx:0,vy:0,life:.45,lightning:true});if(t!==boss&&t.hp<=0)kill(t)}
    if(miniBoss&&!miniBoss.dead&&miniBoss.spawned&&miniBoss.hp<=0){finishMiniBoss();return}if(boss&&!boss.dead&&boss.spawned&&boss.hp<=0){finishBoss();return}
    return;
  }
  if(id==='holy'){
    const radius=w.evolved?180:115;
    for(const e of enemies)if(!e.dead&&dist(e,player)<radius){e.hp-=damage;if(e.hp<=0)kill(e)}
    if(miniBoss&&!miniBoss.dead&&miniBoss.spawned&&dist(miniBoss,player)<radius){miniBoss.hp-=damage*relicDamageMul(miniBoss);if(miniBoss.hp<=0){finishMiniBoss();return}}if(boss&&!boss.dead&&boss.spawned&&dist(boss,player)<radius){boss.hp-=damage*relicDamageMul(boss);if(boss.hp<=0){finishBoss();return}}
...
function spawnMiniBoss(){const hp=4200;miniBoss={x:W/2,y:-70,r:30,maxHp:hp,hp,speed:48,dmg:24,phase:1,shot:1.4,spawned:false,dead:false,intro:2,skill:5,summon:9,flash:0};toast('小BOSS 即将出现！击败它可获得额外奖励');}
function updateMiniBoss(dt){if(!miniBoss||miniBoss.dead)return;if(!miniBoss.spawned){miniBoss.intro-=dt;miniBoss.y=Math.min(H*.22,miniBoss.y+140*dt);if(miniBoss.intro<=0){miniBoss.spawned=true;toast('小BOSS 出现！')}return}let a=Math.atan2(player.y-miniBoss.y,player.x-miniBoss.x);miniBoss.x+=Math.cos(a)*miniBoss.speed*dt;miniBoss.y+=Math.sin(a)*miniBoss.speed*dt;if(dist(miniBoss,player)<miniBoss.r+player.r){player.hp-=miniBoss.dmg*dt*(player.relics?.includes('guard')?.88:1)*(player.synergyHG?.9:1);if(player.hp<=0){if(player.shield){player.shield=false;player.hp=1}else{end();return}}}miniBoss.shot-=dt;miniBoss.skill-=dt;miniBoss.summon-=dt;if(miniBoss.summon<=0){miniBoss.summon=10;for(let i=0;i<2&&enemies.length<MAX_ENEMIES;i++){spawnEnemy();const e=enemies[enemies.length-1];e.x=miniBoss.x+(Math.random()-.5)*140;e.y=miniBoss.y+(Math.random()-.5)*140}}if(miniBoss.skill<=0){miniBoss.skill=6;const radius=110;for(const e of enemies)if(!e.dead&&dist(e,miniBoss)<radius){e.hp-=35;if(e.hp<=0)kill(e)}particles.push({x:miniBoss.x,y:miniBoss.y,vx:0,vy:0,life:.45,bossSkill:true,radius});toast('小BOSS 释放冲击波！')}if(miniBoss.shot<=0){miniBoss.shot=1.8;for(let i=0;i<8;i++){const a=i*Math.PI*2/8;bullets.push({x:miniBoss.x,y:miniBoss.y,a,speed:160,r:6,damage:10,life:2,pierce:0,enemy:true})}}if(miniBoss.hp<=0){finishMiniBoss();return}if(miniBoss.hp<miniBoss.maxHp*.5&&miniBoss.phase===1){miniBoss.phase=2;miniBoss.speed=62;miniBoss.dmg=32;toast('小BOSS 进入狂暴阶段！')}}
function finishMiniBoss(){if(!miniBoss||miniBoss.dead)return;miniBoss.hp=0;miniBoss.dead=true;miniBoss.spawned=false;for(let i=0;i<35&&particles.length<MAX_PARTICLES;i++){const a=Math.random()*Math.PI*2,s=80+Math.random()*260;particles.push({x:miniBoss.x,y:miniBoss.y,vx:Math.cos(a)*s,vy:Math.sin(a)*s,life:.7+Math.random()*.6})}const gold=35;player.gold+=gold;meta.gold+=20;persistMeta();addRandomRelic();toast('小BOSS 已击败！+35 金币 · 永久金币 +20 · 获得遗物');}
function spawnBoss(){
...
function ui(){if(!player)return;$('hp').textContent=Math.ceil(player.hp)+'/'+player.maxHp;$('level').textContent=player.level;$('gold').textContent=player.gold;$('time').textContent=fmt(elapsed);$('hpbar').style.width=Math.max(0,player.hp/player.maxHp*100)+'%';$('xpbar').style.width=Math.min(100,player.xp/player.next*100)+'%';$('weapon').textContent=Object.values(player.weapons).filter(Boolean).map(w=>w.name+' Lv.'+w.level).join(' · ')+(player.relics?.length?' · 遗物 '+player.relics.length:'');if(boss&&!boss.dead){const bb=$('bossbar'),bf=$('bossfill');if(bb)bb.classList.remove('hidden');if(bf)bf.style.width=Math.max(0,boss.hp/boss.maxHp*100)+'%'}else if(miniBoss&&!miniBoss.dead){const bb=$('bossbar'),bf=$('bossfill');if(bb)bb.classList.remove('hidden');if(bf)bf.style.width=Math.max(0,miniBoss.hp/miniBoss.maxHp*100)+'%'}else{const bb=$('bossbar');if(bb)bb.classList.add('hidden')}}
function weaponFx(b){
...
function draw(){
  ...
  if(miniBoss&&!miniBoss.dead){ctx.save();ctx.globalAlpha=miniBoss.flash>0?.65:1;ctx.fillStyle=miniBoss.phase===2?'#ffb347':'#9b6cff';ctx.beginPath();ctx.arc(miniBoss.x,miniBoss.y,miniBoss.r,0,7);ctx.fill();ctx.restore()}
  if(boss&&!boss.dead){
...
function startWatchdog(){if(watchdogStarted)return;watchdogStarted=true;setInterval(()=>{if(state==='playing'&&frameHeartbeat&&performance.now()-frameHeartbeat>1800&&!runtimeErrorShown){reportRuntimeError(new Error('游戏循环停止响应（可能是移动端性能过载）'),'watchdog')}} ,1000)}
...
restoreLocalMeta();startWatchdog();
